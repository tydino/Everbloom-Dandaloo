...
public class ArchaeopteryxEgg extends Block {
    ...
    private static final int HATCHING_TIME = 5000;
    private static final int BOOSTED_HATCHING_TIME = 2500;
    private static final VoxelShape SHAPE = ArchaeopteryxEgg.createCuboidShape(6, 0, 6, 10, 4, 10);

    public MapCodec<ArchaeopteryxEgg> getCodec() {
        return CODEC;
    }

    public ArchaeopteryxEgg(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!this.isReadyToHatch(state)) {
            ...
            ArchaeopteryxEntity baby = (ArchaeopteryxEntity) ModEntities.ARCHAEOPTERYX.create(world, SpawnReason.BREEDING);
            if (baby != null) {
